...
public interface CartItemRepository extends JpaRepository<CartItemEntity, Integer> {
	...
	Optional<Float> calculateFullPrice(@Param ("cartId") Integer cartId);
	
	@Query(value = "select * from cart_items where cart_id = :cartId and book_id = :bookId",nativeQuery = true)
	Optional<CartItemEntity> findByCartIdAndBookId(@Param ("cartId") Integer cartId,@Param ("bookId") Integer bookId);
	
	@Query(value = "select sum(quantity) from cart_items where cart_id=:cartId",nativeQuery = true)
	Optional<Integer> findBookQuantity(@Param ("cartId") Integer cartId);
}
